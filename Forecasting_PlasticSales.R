...
library(smooth)
library(readxl)
Plastics<-read.csv("D:/Data Science Course/Assignments/Forecasting/PlasticSales.csv") # read the Plastic  Data
View(Plastics) 
windows()
...
# Additive Seasonality Linear trend  has least RMSE value

new_model<-lm(log_Sales~t+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec,data = Plasticsdata)
new_model_pred<-data.frame(predict(new_model,interval='predict',newdata=Plasticsdata))

View(new_model_pred)
Month <- as.data.frame(Plasticsdata$Month)
Final <- as.data.frame(cbind(Month,Plasticsdata$Sales, new_model_pred$fit))
