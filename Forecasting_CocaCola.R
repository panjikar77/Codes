...
library(smooth)
library(readxl)
Cocacola<- read_xlsx("D:/Data Science Course/Assignments/Forecasting/CocaCola_Sales_Rawdata.xlsx") # read data
View(Cocacola) 
plot(Cocacola$Sales,type = 'o') # Seasonality 
...
round(Add_sea_Quad_pred, digits = 4)
new <- c("Q3_96", "Q4_69", "Q1_97", "Q2_97")
pred_data <- data.frame(new, round(Add_sea_Quad_pred$fit, digits = 4))
colnames(pred_data) <- c("Quarter", "Sales")

final <- rbind(Cocacola,pred_data)
